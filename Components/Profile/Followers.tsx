...
const wrapFollower = (
  ...
  return (
    <>
      <Tr key={follower.name + "FollowersProfileSection"}>
        <Td>{follower.name}</Td>
        <Td><Center>{follower.uploads.length}</Center></Td>
        <Td><Center>{follower.followers.length}</Center></Td>
        <Td><Center>{follower.following.length}</Center></Td>
        <Td>
          <FollowBack
      ...
      <TableContainer w="90%">
        <Table variant="striped" size="lg">
          <Thead>
            <Tr>
              <Th>Follower </Th>
              <Th><Center>Uploads </Center></Th>
              <Th><Center>Followers </Center></Th>
              <Th><Center>Following</Center> </Th>
            </Tr>
          </Thead>
          <Tbody>
            {followers.map((data) =>
